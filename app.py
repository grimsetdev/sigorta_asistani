...
Müşterinin yazılı kaza beyanı / senaryosu şöyledir: "{beyan}"

Lütfen şu 4 başlıkta detaylı ve acımasız bir rapor hazırla:
1. GÖRÜNÜR HASAR ANALİZİ: Fotoğraftaki fiziksel hasarları detaylandır.
2. KUSUR TAHMİNİ: Kaza kurallarına göre olası kusurlu taraf kimdir?
3. 🚨 SUİSTİMAL (FRAUD) VE TUTARLILIK ANALİZİ: Müşterinin yazılı beyanı ile fotoğraftaki hasar (açı, şiddet, izler) fizik kurallarına göre örtüşüyor mu? Sahte hasar ihbarı veya yalan beyan ihtimali % kaçtır? Eğer tutarsızlık varsa bunu sert bir dille vurgula.
4. HASAR BEYAN DİLEKÇESİ: Sigorta şirketine sunulacak resmi dilekçe metni."""
    try:
        icerik_listesi = [prompt]
        for dosya in gorsel_dosyalari: icerik_listesi.append(Image.open(dosya))
        return client.models.generate_content(model=VISION_MODEL, contents=icerik_listesi).text
    ...
    if st.button("🔍 Hasar Raporunu Oluştur ve Acenteme Gönder", type="primary", use_container_width=True):
        if h_gorseller and h_beyan:
            with st.spinner("Yapay zeka fotoğraflarınızı beyanınızla çapraz sorguluyor..."):
                analiz = kaza_analizi_yap(h_gorseller, st.session_state.musteri_plaka, st.session_state.kullanici_adi, h_beyan)
